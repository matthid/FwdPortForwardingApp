 ...
 */
class UdpForwarder(form: InetSocketAddress, to: InetSocketAddress, ruleName: String) : Forwarder("UDP", form, to, ruleName) {
    @Throws(IOException::class, BindException::class)
    override suspend fun call() {
        Log.d(TAG, kotlin.String.format(Forwarder.START_MESSAGE, protocol, from.port, to.port))
        try {
            ...
            try {
                ...
                Log.e(TAG, String.format(Forwarder.BIND_FAILED_MESSAGE, from.port, protocol, ruleName), e)
                throw BindException(String.format(Forwarder.BIND_FAILED_MESSAGE, from.port, protocol, ruleName), e)
            }
            val selector = Selector.open()
            inChannel.register(selector, SelectionKey.OP_READ, ClientRecord(to))
            while (true) { // Run forever, receiving and echoing datagrams
                if (Thread.currentThread().isInterrupted) {
                    Log.i(TAG, kotlin.String.format(Forwarder.THREAD_INTERRUPT_CLEANUP_MESSAGE, protocol))
    ...
    companion object {
        ...
        private const val TIMEOUT = 3000 // Wait timeout (milliseconds)
